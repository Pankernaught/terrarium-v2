 ...
 *
 * After the base DDL, run the guarded additive ALTERs for `substrate_mix` and
 * `charcoal_depth`: a store created before each feature shipped has every table but
 * lacks that column, and `CREATE TABLE IF NOT EXISTS` can't backfill it. The node
 * test helper mirrors this exact sequence so CI walks the same path.
 */
