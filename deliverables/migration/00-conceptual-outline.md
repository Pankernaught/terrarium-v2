...
**Status:** skeleton (Pass 1, complete). This is the top of a planning ladder. Live ladder state, per-phase progress, and the next-chat kickoff live in the single `deliverables/migration/HANDOFF-*.md` file (currently `HANDOFF-execute-M0.md`) — read that first.
...
- **A4 — `builds` "layer depths / mix" mapping is imprecise (§9 table, §4.3).** Layer depths are **scalar `real` columns** (`substrateDepth`/`drainageDepth`/`charcoalDepth`), not JSON; `substrateMix` and `careOverrides` are JSON **objects**, not arrays. **M9's field mapping should reflect this exactly.**
